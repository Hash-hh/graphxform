...
class GumbeldoreDataset:
    ...
    def generate_dataset(self, network_weights: dict, best_objective: Optional[float] = None,
        ...
        if getattr(self.config, 'disable_ray', False):
            results = self._run_sequential(
                problem_instances, network_weights, best_objective, memory_aggressive
            )
        else:
    ...
    def _run_sequential(self, problem_instances, network_weights, best_objective, memory_aggressive):
        """Sequential fallback for debugging without Ray. Runs all instances on a single device."""
        ...
        objective_evaluator = MoleculeObjectiveEvaluator(
            config, torch.device(config.objective_gnn_device),
            oracle_tracker=self.oracle_tracker
        )

        autocast_ctx, _ = _make_autocast_ctx(config) if config.use_amp_inference else (nullcontext(), None)

        def child_log_probability_fn(trajectories):
            with autocast_ctx:
                return MoleculeDesign.log_probability_fn(trajectories=trajectories, network=network)

        def batch_leaf_evaluation_fn(trajectories):
        ...
        with torch.no_grad():
            for idx, root_node in tqdm(enumerate(problem_instances), total=len(problem_instances),
                                       desc="[Sequential] Generating"):
                root_nodes = [root_node]

                if config.gumbeldore_config["search_type"] == "iid_mc":
                    num_samples = config.gumbeldore_config.get("num_samples_per_instance", 512)
                    trajectories_by_root = batched_iid_monte_carlo_sampling(
                        root_nodes, num_samples, child_log_probability_fn, config
                    )
                    ...
                    results[idx] = trajectories_by_root[0] if trajectories_by_root else []
                    ...
                    result = [x.state for x in beam_leaves_batch[0]]
                    ...
                    inc_sbs = IncrementalSBS(
                        root_nodes, child_log_probability_fn, child_transition_fn,
